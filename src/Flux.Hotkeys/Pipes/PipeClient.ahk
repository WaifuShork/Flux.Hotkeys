...
class PipeClient
{
    ...
    IsConnected()
    {
        b := this.GetBytesToRead() ; call the peeknamedpipe, in order to check last error
    ...
    WaitForDataToRead(max_wait_time := 0)
    {
      waited := 0
      step := 100

      buffer_size := this.GetBytesToRead()
    ...
    Read()
    {
        wait_result := this.WaitForDataToRead() ;wait infintely for data
